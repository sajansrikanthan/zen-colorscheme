
local c = require('zen.palette')

local hl = vim.api.nvim_set_hl
local theme = {}

theme.set_highlights = function()

  -- highlights
  hl(0, "Normal", { fg = c.fg, bg = c.bg })
  hl(0, "SignColumn", { fg = 'NONE', bg = c.bg })
  hl(0, "MsgArea", { fg = c.fg, bg = c.bg })
  hl(0, "ModeMsg", { fg = c.fg, bg = c.bg })
  hl(0, "MsgSeparator", { fg = c.fg, bg = c.bg })
  hl(0, "NormalNC", { fg = c.fg, bg = c.bg })
  hl(0, "Pmenu", { fg = c.fg, bg = c.bg, sp = 'NONE', blend=50,  })
  hl(0, "PmenuSel", { fg = c.bg, bg = c.fg })
  hl(0, "CursorLineNr", { fg = c.fg, bg = 'NONE' })
  hl(0, "LineNrAbove", { fg = c.sfg, bg = 'NONE' })
  hl(0, "LineNrBelow", { fg = c.sfg, bg = 'NONE' })
  hl(0, "Comment", { fg = c.sfg, bg = 'NONE' })
  hl(0, "Whitespace", { fg = c.bg, bg = 'NONE' })
  hl(0, "CursorLine", { fg = 'NONE', bg = c.bg })
  hl(0, "CursorColumn", { fg = 'NONE', bg = c.bg })
  hl(0, "ColorColumn", { fg = 'NONE', bg = c.bg })
  hl(0, "NormalFloat", { fg = 'NONE', bg = c.bg })
  hl(0, "Visual", { fg = 'NONE', bg = c.visualline })
  hl(0, "WarningMsg", { fg = c.red, bg = c.bg })
  hl(0, "Cursor", { fg = c.cursor_fg, bg = c.cursor_bg })
  hl(0, "lCursor", { fg = c.cursor_fg, bg = c.cursor_bg })
  hl(0, "CursorIM", { fg = c.cursor_fg, bg = c.cursor_bg })
  hl(0, "TermCursor", { fg = c.cursor_fg, bg = c.cursor_bg })
  hl(0, "TermCursorNC", { fg = c.cursor_fg, bg = c.cursor_bg })
  hl(0, "Title", { fg = c.fg, bg = 'NONE', bold=true, })
  hl(0, "EndOfBuffer", { fg = c.purple, bg = 'NONE' })
  hl(0, "Variable", { fg = c.fg, bg = 'NONE' })
  hl(0, "Constant", { fg = c.fg, bg = 'NONE' })
  hl(0, "String", { fg = c.fg, bg = 'NONE' })
  hl(0, "Character", { fg = c.fg, bg = 'NONE' })
  hl(0, "Number", { fg = c.fg, bg = 'NONE' })
  hl(0, "Boolean", { fg = c.fg, bg = 'NONE' })
  hl(0, "Float", { fg = c.fg, bg = 'NONE' })
  hl(0, "Identifier", { fg = c.fg, bg = 'NONE' })
  hl(0, "Function", { fg = c.fg, bg = 'NONE' })
  hl(0, "Operator", { fg = c.fg, bg = 'NONE' })
  hl(0, "Type", { fg = c.fg, bg = 'NONE' })
  hl(0, "StorageClass", { fg = c.fg, bg = 'NONE' })
  hl(0, "Structure", { fg = c.fg, bg = 'NONE' })
  hl(0, "Typedef", { fg = c.fg, bg = 'NONE' })
  hl(0, "Keyword", { fg = c.fg, bg = 'NONE' })
  hl(0, "Statement", { fg = c.fg, bg = 'NONE' })
  hl(0, "Conditional", { fg = c.fg, bg = 'NONE' })
  hl(0, "Repeat", { fg = c.fg, bg = 'NONE' })
  hl(0, "Label", { fg = c.fg, bg = 'NONE' })
  hl(0, "Exception", { fg = c.fg, bg = 'NONE' })
  hl(0, "Include", { fg = c.fg, bg = 'NONE' })
  hl(0, "PreProc", { fg = c.fg, bg = 'NONE' })
  hl(0, "Define", { fg = c.fg, bg = 'NONE' })
  hl(0, "Macro", { fg = c.fg, bg = 'NONE' })
  hl(0, "PreCondit", { fg = c.fg, bg = 'NONE' })
  hl(0, "Special", { fg = c.fg, bg = 'NONE' })
  hl(0, "SpecialChar", { fg = c.fg, bg = 'NONE' })
  hl(0, "Tag", { fg = c.fg, bg = 'NONE' })
  hl(0, "Bold", { fg = 'NONE', bg = 'NONE', bold=true, })
  hl(0, "Italic", { fg = 'NONE', bg = 'NONE', italic=true, })
  hl(0, "Ignore", { fg = c.fg, bg = c.bg })
  hl(0, "Todo", { fg = c.fg, bg = c.bg })
  hl(0, "Error", { fg = c.red, bg = c.bg })

  -- Treesitter
  hl(0, "TSComment", { link = 'Comment' })
  hl(0, "TSAnnotation", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSAttribute", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSConstructor", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSType", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSTypeBuiltin", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSConditional", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSException", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSInclude", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSKeywordReturn", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSKeyword", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSKeywordFunction", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSLabel", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSNamespace", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSRepeat", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSConstant", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSConstBuiltin", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSFloat", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSNumber", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSBoolean", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSCharacter", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSError", { fg = c.red, bg = 'NONE' })
  hl(0, "TSFunction", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSFuncBuiltin", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSMethod", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSConstMacro", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSFuncMacro", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSVariable", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSVariableBuiltin", { fg = c.red, bg = 'NONE' })
  hl(0, "TSProperty", { fg = c.red, bg = 'NONE' })
  hl(0, "TSField", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSParameter", { fg = c.red, bg = 'NONE' })
  hl(0, "TSParameterReference", { fg = c.red, bg = 'NONE' })
  hl(0, "TSSymbol", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSText", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSOperator", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSPunctDelimiter", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSTagDelimiter", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSTagAttribute", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSPunctBracket", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSPunctSpecial", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSString", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSStringRegex", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSStringEscape", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSTag", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSEmphasis", { fg = 'NONE', bg = 'NONE', italic=true, })
  hl(0, "TSUnderline", { fg = 'NONE', bg = 'NONE', underline=true, })
  hl(0, "TSTitle", { fg = c.fg, bg = 'NONE',  })
  hl(0, "TSLiteral", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSURI", { fg = c.fg, bg = 'NONE', underline=true, })
  hl(0, "TSKeywordOperator", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSStructure", { fg = c.fg, bg = 'NONE' })
  hl(0, "TSStrong", { fg = c.fg, bg = 'NONE', bold=true, })
  hl(0, "TSQueryLinterError", { fg = c.fg, bg = 'NONE' })
  hl(0, "TreesitterContext", { fg = 'NONE', bg = c.bg })

  -- markdown
  hl(0, "markdownBlockquote", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownCode", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownCodeBlock", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownCodeDelimiter", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownH1", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownH2", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownH3", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownH4", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownH5", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownH6", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownHeadingDelimiter", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownHeadingRule", { fg = c.fg, bg = 'NONE', bold=true, })
  hl(0, "markdownId", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownIdDeclaration", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownIdDelimiter", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownLinkDelimiter", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownBold", { fg = c.fg, bg = 'NONE', bold=true, })
  hl(0, "markdownItalic", { fg = 'NONE', bg = 'NONE', italic=true, })
  hl(0, "markdownBoldItalic", { fg = c.fg, bg = 'NONE', bold=true, italic=true, })
  hl(0, "markdownListMarker", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownOrderedListMarker", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownRule", { fg = c.red, bg = 'NONE' })
  hl(0, "markdownUrl", { fg = c.fg, bg = 'NONE', underline=true, })
  hl(0, "markdownLinkText", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownFootnote", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownFootnoteDefinition", { fg = c.fg, bg = 'NONE' })
  hl(0, "markdownEscape", { fg = c.red, bg = 'NONE' })

  -- Git
  hl(0, "SignAdd", { fg = c.fg, bg = 'NONE' })
  hl(0, "SignChange", { fg = c.fg, bg = 'NONE' })
  hl(0, "SignDelete", { fg = c.red, bg = 'NONE' })
  hl(0, "GitSignsAdd", { fg = c.green, bg = 'NONE' })
  hl(0, "GitSignsChange", { fg = c.orange1, bg = 'NONE' })
  hl(0, "GitSignsDelete", { fg = c.red, bg = 'NONE' })

  -- LSP
  hl(0, "LspDiagnosticsDefaultError", { fg = c.red, bg = 'NONE' })
  hl(0, "LspDiagnosticsDefaultWarning", { fg = c.orange2, bg = 'NONE' })
  hl(0, "LspDiagnosticsDefaultInformation", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsDefaultInfo", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsDefaultHint", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsVirtualTextError", { fg = c.red, bg = 'NONE' })
  hl(0, "LspDiagnosticsVirtualTextWarning", { fg = c.orange2, bg = 'NONE' })
  hl(0, "LspDiagnosticsVirtualTextInformation", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsVirtualTextInfo", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsVirtualTextHint", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsFloatingError", { fg = c.red, bg = 'NONE' })
  hl(0, "LspDiagnosticsFloatingWarning", { fg = c.orange2, bg = 'NONE' })
  hl(0, "LspDiagnosticsFloatingInformation", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsFloatingInfo", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsFloatingHint", { fg = c.fg, bg = 'NONE' })
  hl(0, "DiagnosticSignError", { fg = c.red, bg = 'NONE' })
  hl(0, "DiagnosticSignWarning", { fg = c.orange2, bg = 'NONE' })
  hl(0, "DiagnosticSignInformation", { fg = c.fg, bg = 'NONE' })
  hl(0, "DiagnosticSignInfo", { fg = c.fg, bg = 'NONE' })
  hl(0, "DiagnosticSignHint", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsSignError", { fg = c.red, bg = 'NONE' })
  hl(0, "LspDiagnosticsSignWarning", { fg = c.orange2, bg = 'NONE' })
  hl(0, "LspDiagnosticsSignInformation", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsSignInfo", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsSignHint", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsError", { fg = c.red, bg = 'NONE' })
  hl(0, "LspDiagnosticsWarning", { fg = c.orange2, bg = 'NONE' })
  hl(0, "LspDiagnosticsInformation", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsInfo", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsHint", { fg = c.fg, bg = 'NONE' })
  hl(0, "LspDiagnosticsUnderlineError", { fg = 'NONE', bg = 'NONE', underline=true, })
  hl(0, "LspDiagnosticsUnderlineWarning", { fg = 'NONE', bg = 'NONE', underline=true, })
  hl(0, "LspDiagnosticsUnderlineInformation", { fg = 'NONE', bg = 'NONE', underline=true, })
  hl(0, "LspDiagnosticsUnderlineInfo", { fg = 'NONE', bg = 'NONE', underline=true, })
  hl(0, "LspDiagnosticsUnderlineHint", { fg = 'NONE', bg = 'NONE', underline=true, })
  hl(0, "LspReferenceRead", { fg = 'NONE', bg = '#2e303b' })
  hl(0, "LspReferenceText", { fg = 'NONE', bg = '#2e303b' })
  hl(0, "LspReferenceWrite", { fg = 'NONE', bg = '#2e303b' })

  -- Telescope
  hl(0, "TelescopeSelection", { fg = c.purple, bg = 'NONE' })
  hl(0, "TelescopeMatching", { fg = c.orange2, bg = 'NONE', bold=true, })
  hl(0, "TelescopeBorder", { fg = c.orange1, bg = c.bg })

  -- StatusLine
  hl(0, "StatusLine", { fg = c.fg, bg = c.bg })
  hl(0, "StatusLineNC", { fg = c.fg, bg = c.bg })
  hl(0, "StatusLineTerm", { link = 'StatusLine' })
  hl(0, "StatusLineTermNC", { link = 'StatusLineNC' })
end

return theme